...
var svgArea, svgShapes, ButtonDeleteActiveHandlePoint; 
...
jQ( document ).ready(function() {
	...
	if (SVG.supported) 
	{
		...
		svgArea.shapeObjects.drawPolygon = function ()
		{
		...
		svgArea.shapeObjects.deleteActiveHandlePoint = function ()
		{
			if(	svgArea.activeHandlePoint != null)
			{
				svgArea.activeHandlePoint.remove(); //remove the internal svg and downstream update the polygon entry
				delete svgArea.activeHandlePoint; //delete the object refered to in svgArea.activeHandlePoint, svgArea.activeHandlePoint will become undefined
				svgArea.activeHandlePoint = null; //reset to null
			}
		}
	} 
	else 
	...
	jQ('button#btDrawPolygon').click(function(){
		...
		svgArea.shapeObjects.drawPolygon();
	});
	
	// button delete polygon action
	jQ('button#btDeletePoint').click(function(){
		svgArea.shapeObjects.deleteActiveHandlePoint();
	});
	
	// buttons dynamic styling
	ButtonDeleteActiveHandlePoint = {};
	
	//sets the sytle on button 'delete active handle point' that indicates that there is a handlpoint cative, thus may be deleted
	ButtonDeleteActiveHandlePoint.setDeleteActive = function()
	{
		jQ('button#btDeletePoint').addClass('deleteActive');
	}

	//sets the sytle on button 'delete active handle point' that indicates that there is no handlpoint cative, thus none can be deleted
...
*/
	...
	/*
	 * deletes the handlePoint: remove the internal svg and downstream update the polygon entry
	 */
	this.remove = function()
	{
		alert("remove point")
		//remove from any parentshape that may old it
		
		//remove svg circle
		
		//remove from svgShapes
